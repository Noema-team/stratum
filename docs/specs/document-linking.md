...
9. **System states do not block indexing.** The link index updates regardless
   of `meta.status` (idle, discovering) or any `WorkflowRun.status`. Link
   indexing never waits for a state transition. This includes checkpoint
   pauses (`WorkflowRun.awaiting_checkpoint` set) — the workflow runner
...
11. **No link versioning, narrow staleness exception.** Links are not versioned
    alongside artifacts. When an artifact is modified, its links are
    re-extracted from the new content. Old links are replaced, not preserved.
    The workflow-run event log (map.yaml → `workflow_runs.{run_id}.steps_completed`)
    provides the historical provenance record. The one carve-out: a link may
    record the artifact version it pointed at when created, used only to flag
    a link as stale if the target has since been committed at a newer
    version — this is a staleness pointer, not link-history/versioning.
    (DDR-031; resolves DL-001 below)

12. **Manual link ownership.** Manual links carry `created_by: 'user'`. When
    the source artifact is regenerated by a workflow run, manual links embedded in
...
|---|---|---|---|
| DL-001 | Should the link index support link versioning to trace how relationships evolved across workflow runs? | History tracking, audit trail | Addressed by DDR-031 (narrow staleness pointer, not full link versioning) |
