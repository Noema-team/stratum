...
4. **Passed categories** — single line each (no detail)
5. **Static analysis** — from `static-analysis/results.json`
...
|---|---|---|
| `correctness` | requirements.md, src/**/*, charter (summary) | Intent match, edge cases, data contracts |
| `performance` | requirements.md, architecture.md, charter (summary) | Bottlenecks, caching, async patterns |
| `security` | requirements.md, architecture.md, src/**/*, charter (summary) | Auth boundaries, input validation, secrets |
| `usability` | requirements.md, src/**/*, charter (summary) | Error messages, discoverability, defaults |
| `reliability` | requirements.md, architecture.md, charter (summary) | Retries, graceful degradation, timeouts |
| `maintainability` | src/**/*, architecture.md, charter (summary) | Complexity, module cohesion, coverage |
