...
| [ddr-033-technology-independent-software-lifecycle.md](ddr-033-technology-independent-software-lifecycle.md) | Technology-independent software lifecycle |
| [ddr-034-models-propose-stratum-materializes.md](ddr-034-models-propose-stratum-materializes.md) | Models produce typed semantic proposals; Stratum decodes, validates, derives control, and materializes canonical artifact bytes (OutputContract seam, readiness-first, deterministic rendering) |
| [ddr-035-post-d34-operational-pivot.md](ddr-035-post-d34-operational-pivot.md) | Post-D.34 operational pivot: Era I (trustworthy agent boundary) complete; Era II begins — pilots create the milestones, never patch mid-pilot, evidence before architecture, ci-toolkit stays external |
| [ddr-036-escalation-ownership.md](ddr-036-escalation-ownership.md) | Escalation ownership (evidence-triggered DDR-034 completion): escalation identity and provenance are Stratum-owned — readiness gap factId, decision-request targetFactId, decision-application merge, exploration-need contract; models classify and propose consequences, never copy identity |
| [ddr-037-instrument-closure.md](ddr-037-instrument-closure.md) | Qualification instrument closure: a finite selection Stratum can enumerate (readiness factId domain) is exposed as one — initial candidate-list teaching via a pure contextTeaching projection, schema annotation, and repair messages that list the legal set; MATURE fixture supplies its documented repository knowledge while the iteration-1 criterion stands |
| [ddr-038-instrument-validity.md](ddr-038-instrument-validity.md) | Qualification instrument validity: the EARLY option oracle adjudicates the intended outcome (excludes cross-platform from this increment) instead of matching vocabulary — deterministic polarity evaluation pinned on historical payloads; per-run informational isolation (fresh root, in-memory state) verified null-leak and pinned by regression |
| [ddr-039-oracle-identity-join.md](ddr-039-oracle-identity-join.md) | Oracle identity join: the platform-scope Decision is selected by durable `subjectRef.targetFactId` equality with the semantically-identified platform-scope fact — fail-closed on zero/multiple matches — replacing lexical Decision-text selection (E4-G inv 4 adversarial pair pinned); the audited-not-fixed production re-bind gap for UNLINKED decisions is recorded for adjudication |
| [ddr-040-unlinked-decision-recovery.md](ddr-040-unlinked-decision-recovery.md) | Decision creation-time identity propagation: every dynamic Decision-creation path (Scheduler dispatch AND ResumeService next-checkpoint) copies the validated request's `targetFactId` into the durable `subjectRef` at birth — the asymmetric propagation was E4-G inv 4's root cause, not model omission; identity stays immutable thereafter, legacy/unlinked Decisions keep the deterministic fail-closed refusal |
| [ddr-041-definition-execution-handoff.md](ddr-041-definition-execution-handoff.md) | Trusted Definition → execution handoff: an execution WorkItem names the exact completed define-work WorkItem (`workflowParameters.definitionSource`); the system resolves it through D.1 artifact provenance (exactly one `definition` artifact, same project, matching Objective, `completed` state) and sha256-pins the canonical bytes as authoritative context before any step runs — frozen into `WorkflowRun.resolvedParameters`, byte-identical across resume, rendered verbatim under its own AUTHORITATIVE DEFINITION header, zero/ambiguous/mutated/oversized all fail closed; authorized by the E5 preflight that statically falsified H1 (no operator translation, no chaining subsystem) |
