...
| [ddr-040-unlinked-decision-recovery.md](ddr-040-unlinked-decision-recovery.md) | Unlinked decision recovery: every Decision-creation path binds `targetFactId` at birth (root cause — ResumeService's next-checkpoint creation dropped it, E4-G inv 4), and `WorkflowStep.on_error_routes` gives `DECISION_APPLICATION_DECISION_UNLINKED` one bounded authority re-cycle (apply→prepare) under a durable per-run budget (migration 11); Decision identity is never mutated or transferred, every other defect stays fail-closed |
