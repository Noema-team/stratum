...
| C5 | `submit_result` transport: tool channel on `completeMultiTurn` for produce steps; tool schema from `resultSchemaJson`; negotiation in `resolveResultTransport` extended; negotiated transport recorded in run metadata | Multi-turn produce with read tools + submission on OpenRouter and Anthropic paths; rejection continuation via `tool_result` | **DONE — merged `c0d0216` (PR #7)** |
| C6 | `completeStructured` provider capability + review-step structured channel | Capability-probed; textual fallback intact everywhere | **IN REVIEW** |
...
| 2026-09-11 | *(this commit)* | C4 Definition contract + wiring — IN REVIEW: `definition-contract.ts` (`DefinitionProposal`, strict zod schema, `renderDefinition` with the pinned dump options + §10 invariants, `definitionProposalFromPersisted` load-path helper, `validateDefinitionProposal` wrapping `validateDefinition` VERBATIM with the composition-root `findDecision` closure + same-work-item ownership, `createDefinitionOutputContract`); `definition` registered at the composition root sharing ONE `findDecision` closure across the input gate and the contract. **Multi-turn gate requirement met:** proposal mode on the multi-turn textual path — same `SLE-OUTPUT` delimiters (the loop's compliance signal), a single JSON payload inside (no `### <path>` section, no bytes), absent-vs-malformed taxonomy preserved, mode-correct repair; proven E2E through `buildAgentRunner` with a provider that actually exposes `completeMultiTurn` (success, in-conversation result repair, fail-closed exhaustion, and a non-contract step staying legacy-materialized on the same runner). Serialization teaching moved out of `DEFINITION_CONTRACT` into the projection (§8.3) — meaning kept: two-part structure, section semantics, constraint vocabulary, deterministic gate, epistemic rules verbatim; `writeGateRejection` + refine repair asks made representation-neutral. C3's "produce stays legacy" E2E repurposed to the surviving invariant (no-contract types stay legacy). **Review closures (same PR):** (1) the ENTIRE generated provider-facing JSON Schema is golden-pinned (the C2 rule — the projection is the runtime protocol); (2) multi-turn proposal cardinality is fail-closed — exactly one SLE-OUTPUT block per reply, multiple/stray blocks are malformed and take bounded format repair (regressions: two complete blocks, trailing unclosed block, second closer, single-block control). **MERGED** `2558223` (PR #6, ACCEPTED). |
| 2026-09-11 | *(this commit)* | C5 submit-result transport — IN REVIEW: `SubmitResultTransport` (tool channel on `completeMultiTurn` for contract steps; tool input schema = the contract projection via `resultSchemaJson`; exactly-once submission taught; end_turn without submission = `absent` format defect; fail-closed cardinality — the C4 rule — on the tool channel); negotiation in `resolveResultTransport` (schema-carrying step + genuine multi-turn capability → submit-result; everything else textual; explicit override always wins); the runner forwards ONLY an explicitly configured transport override to the loop (its resolved default no longer outranks negotiation); rejection continuation via `tool_result` answering EVERY tool_use in the turn; negotiated transport recorded in loop metadata (`result_transport`). C5 gate held: only the wire changed — same acceptor, same budgets, same materialization/routing/provenance (E2E asserts renderer-identical bytes through the tool channel). Wire shapes proven on BOTH provider paths (Anthropic SDK `input_schema`; OpenRouter/OpenAI `parameters`) with the loop's shared `tool_use` mapping. C4's multi-turn E2Es re-pinned as textual-fallback regressions via explicit override. **Review closure:** `submit_result` is TERMINAL and EXCLUSIVE for its turn — a turn carrying a read request alongside the submission is `malformed` (every tool_use answered, format-repair budget, no result accepted from that turn); teaching tightened to "investigate on EARLIER turns, then call `submit_result` alone". **MERGED** `c0d0216` (PR #7, ACCEPTED). |
| 2026-09-11 | *(this commit)* | C6 native structured output — IN REVIEW: `IStructuredProvider.completeStructured` capability seam (structural probe, synced honestly by `DynamicLLMProvider` exactly like multi-turn — fallback by CAPABILITY, never provider name); genuine implementations: OpenAI-wire `response_format json_schema` strict (OpenRouter case opts in; openai_compatible/glm stay without) and Anthropic forced single-tool extraction (choice FORCED, single-shot — never a tool loop, multi-turn review not reopened); the runner's single-turn branch slots the structured wire in for schema-carrying steps: schema = the contract projection verbatim (`schemaName` = the declared artifact type), field-meaning annotations ride in the message while NO envelope syntax is taught, `complete()` never called on this wire, result repair re-issues the same conversation shape with the same budget/counter, non-JSON from a structured call fails closed. C6 gate proven: structured vs textual review channels → byte-identical readiness bytes, identical typed verdict/route, identical provenance. Boundary pinned: produce steps on providers with BOTH capabilities stay on `submit_result` (structured = review channel only). |
